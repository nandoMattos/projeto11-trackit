...
export default function TodayPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/habits/today`, config)
      .then((res) => {
        setIsLoading(false);
        ...
        <header>
          <HeaderDiv>
            <h1 data-identifier="today-infos">
              {todayWeekday.weekday}, {dayjs().format("DD/MM")}
            </h1>
          ...
          {userProgress !== 0 ? (
            <Feedback color={CHECKED_GREEN} data-identifier="today-infos">
              {Math.round(userProgress)}% dos hábitos concluídos
            </Feedback>
          ) : (
            <Feedback color={TEXT_COLOR} data-identifier="today-infos">
